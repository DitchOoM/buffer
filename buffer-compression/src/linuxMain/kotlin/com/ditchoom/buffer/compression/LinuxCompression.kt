...
// Linux native zlib exposes deflateSetDictionary/inflateSetDictionary directly.
...
actual fun decompress(
    ...
    try {
        ...
        val remaining = buffer.remaining()
        if (remaining == 0) {
            CompressionResult.Success(BufferFactory.deterministic().allocate(0))
        } else {
...
private fun compressWithZStream(
    ...
    val output = BufferFactory.deterministic().allocate(maxOutputSize, ByteOrder.BIG_ENDIAN)
...
private fun decompressWithZStream(
    ...
    try {
        ...
        var output = BufferFactory.deterministic().allocate(outputSize, ByteOrder.BIG_ENDIAN)
                ...
                when (result) {
                    ...
                    Z_OK, Z_BUF_ERROR -> {
                        ...
                        if (s.avail_out == 0u) {
                            ...
                            val newOutput = BufferFactory.deterministic().allocate(newSize, ByteOrder.BIG_ENDIAN)
...
private fun createEmptyCompressed(algorithm: CompressionAlgorithm): PlatformBuffer {
    ...
    val buffer = BufferFactory.deterministic().allocate(bytes.size, ByteOrder.BIG_ENDIAN)
