...
| An **unframed generic dispatcher**'s `peekFrameSize` stays instance-only. Its peek routes through per-variant receivers, and a generic variant's is a constructor-injected instance field, so hoisting it needs each variant's framing status carried on the dispatch IR. (A **framed** generic dispatcher does not have this problem — its peek is one collapsed header+prefix walk — and was hoisted in #348.) | documented limitation (instance framing unchanged; a consumer of an unframed generic sealed parent still needs the dispatcher instance to peek) | `buildDispatchPeekFun` (`variant.codecReceiver()`); `DispatchVariant` |
...
| A message that delegates framing to its **own companion's** `FrameDetector` is classified `PeekEmit.Framed` on that declaration alone, so it gets the companion entry point like any framed shape — but the delegate may still answer `NoFraming` at runtime. This is the one hole in "the companion carries framing ⇒ this shape frames": everywhere else that implication is a compile-time fact derived by the walker. | deliberate escape hatch (the processor cannot inspect a consumer's detector; what it returns is the consumer's own contract) | `detectCustomFramePeek` (`CodecAnalyzer.kt`); `CodecShape.customPeek` → `PeekEmit.Framed` in `buildPeekFrame`; `PeekEmit.Framed` kdoc |
