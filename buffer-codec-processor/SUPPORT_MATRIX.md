...
| Companion-side `peekFrameSize` on a generic codec, so framing needs no `Codec<P>` instance (#348); the member forwards to it. Applied to **every** generic codec whose peek body is codec-independent, framing or not — placement matches an `object` codec's, and `PeekResult.NoFraming` stays a runtime answer rather than a shape-dependent compile error | snapshots `deferredpayload/SmpGenericFrameCodec.kt` (frames), `websocket/WsFrameTextCodec.kt` (`NoFraming`); `addCodecCompanion`, `memberPeekFun` |
| Same companion hoist for a **`@FramedBy` generic dispatcher**, merged into the existing `decodeAggregating` companion when the discriminator is a value class | snapshots `mqtt/MqttPacketCodec.kt`, `slice14cgeneric/Slice14cGenericFramedDispatchCodec.kt`; `CodecCompanion.AggregatorAndFraming` |
...
| An **unframed generic dispatcher**'s `peekFrameSize` stays instance-only — the one codec shape not hoisted to a companion. Its peek routes through per-variant receivers, and a generic variant's is a constructor-injected instance field, so the body genuinely cannot compile in companion scope; hoisting it needs each variant's framing status carried on the dispatch IR. (A **framed** generic dispatcher has no such problem — its peek is one collapsed header+prefix walk naming only type names — and was hoisted in #348.) | documented limitation (instance framing unchanged; a consumer of an unframed generic sealed parent still needs the dispatcher instance to peek) | `buildDispatchPeekFun` (`variant.codecReceiver()`); `requireCodecIndependentPeek` rejects this body if it is ever passed for hoisting |
...
| Whether a codec frames is not knowable from its type — `peekFrameSize` resolves on every codec (`Codec` extends `FrameDetector`, whose default answer is `NoFraming`), so an unframable shape is a runtime `PeekResult.NoFraming`, never a compile error. Consumers must handle that arm. | by design (`PeekResult` is sealed; `NoFraming` is one of its cases). Making framing a static fact means removing `FrameDetector` from `Codec` and omitting the member on unframable shapes — tracked as #358 (v7). | `FrameDetector.peekFrameSize` default in `Codec.kt`; `PeekResult.NoFraming` |
