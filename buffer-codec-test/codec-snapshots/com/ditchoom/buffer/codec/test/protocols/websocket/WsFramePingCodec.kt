...
public class WsFramePingCodec<P : Payload>(
  ...
  override fun decode(buffer: ReadBuffer, context: DecodeContext): WsFrame.Ping<P> {
    val __batch21 = buffer.readShort().toInt() and 0xFFFF
    ...
    if (buffer.byteOrder == ByteOrder.BIG_ENDIAN) {
      byte1 = FrameHeaderByte1((__batch21 ushr 8 and 0xFF).toUByte())
      byte2 = WsHeaderByte2((__batch21 and 0xFF).toUByte())
    } else {
      byte1 = FrameHeaderByte1((__batch21 and 0xFF).toUByte())
      byte2 = WsHeaderByte2((__batch21 ushr 8 and 0xFF).toUByte())
    }
  ...
  public companion object {
    public fun <P : Payload> partial(buffer: ReadBuffer, context: DecodeContext): Partial<P> {
      ...
      val byte2: com.ditchoom.buffer.codec.test.protocols.websocket.WsHeaderByte2
      if (buffer.byteOrder == ByteOrder.BIG_ENDIAN) {
        byte1 = FrameHeaderByte1((__batch22 ushr 8 and 0xFF).toUByte())
        byte2 = WsHeaderByte2((__batch22 and 0xFF).toUByte())
      } else {
        byte1 = FrameHeaderByte1((__batch22 and 0xFF).toUByte())
        byte2 = WsHeaderByte2((__batch22 ushr 8 and 0xFF).toUByte())
      }
      val extendedLength16: UShort? = if (byte2.extended16) buffer.readUShort() else null
      val extendedLength64: Long? = if (byte2.extended64) buffer.readLong() else null
      val maskingKey: WsMaskingKey? = if (byte2.masked) WsMaskingKey(buffer.readUInt()) else null
