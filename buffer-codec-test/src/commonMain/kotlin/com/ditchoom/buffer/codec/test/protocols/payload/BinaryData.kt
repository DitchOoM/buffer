 ...
 * (Change 1): the inner storage is an [OwnedBytesHandle], not a raw
 ...
 *
 * **Test ergonomics**: use [ownedBytesOf] to construct from a `ByteArray`
 * literal, and [OwnedBytesHandle.toBytes] to materialize back via
...
value class BinaryData(
     ...
     * production code should prefer [OwnedBytesHandle.asReadBuffer] for
     * the zero-copy view, or [OwnedBytesHandle.handleEquals] for content
...
object BinaryDataCodec : Codec<BinaryData> {
    override fun decode(
        ...
        dst.write(buffer)
        dst.resetForRead()
        return BinaryData(ownedBytesFrom(dst))
    }
 ...
 * literal. Wraps via [ownedBytesOf] (which uses [testFixtureFactory]).
 */
fun BinaryData(bytes: ByteArray): BinaryData = BinaryData(ownedBytesOf(bytes))
