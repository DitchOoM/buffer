...
class MqttPubRelCodecTest {
    ...
    fun wireSizeIsBackPatchWithUseCodecScalar() {
        val msg = MqttPacket.PubRel(packetIdentifier = 1u)
        assertEquals(WireSize.BackPatch, PubRelCodec.wireSize(msg, EncodeContext.Empty))
    }
    ...
    private fun encodeAndAssertBytes(
        ...
        val buf = encode(msg)
        assertEquals(expected.size, buf.position(), "encoded byte count matches MQTT-3.1.1 §3.6 layout")
        buf.resetForRead()
    ...
    private fun encode(value: MqttPacket.PubRel) =
        BufferFactory.Default
            .allocate(8, ByteOrder.BIG_ENDIAN)
            .also { PubRelCodec.encode(it, value, EncodeContext.Empty) }
}
