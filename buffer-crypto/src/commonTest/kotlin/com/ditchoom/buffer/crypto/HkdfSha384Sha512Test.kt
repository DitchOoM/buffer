...
class HkdfSha384Sha512Test {
    ...
    @Test
    fun hkdfSha512Tc1() {
        val okm = HkdfSha512.derive(salt, ikm, info, 42, BufferFactory.Default)
        assertEquals(
    ...
    @Test
    fun hkdfSha384Tc1() {
        val okm = HkdfSha384.derive(salt, ikm, info, 42, BufferFactory.Default)
        assertEquals(
    ...
    fun hkdfSha512NullSaltAndInfo() {
        val okm = HkdfSha512.derive(salt = null, ikm = ikm, info = null, length = 42, factory = BufferFactory.Default)
        assertEquals(
    ...
    fun maxLengthBoundary() {
        ...
        assertEquals(max512, HkdfSha512.derive(salt, ikm, info, max512, BufferFactory.Default).remaining())
        assertFailsWith<IllegalArgumentException> {
            HkdfSha512.derive(salt, ikm, info, max512 + 1, BufferFactory.Default)
        }
        val max384 = 255 * SHA384_DIGEST_BYTES
        assertEquals(max384, HkdfSha384.derive(salt, ikm, info, max384, BufferFactory.Default).remaining())
        assertFailsWith<IllegalArgumentException> {
            HkdfSha384.derive(salt, ikm, info, max384 + 1, BufferFactory.Default)
        }
    }
}
