...
/**
 * Native implementation using a pure Kotlin fallback.
 *
 * For full compression support, consider linking to zlib.
...
private fun createDeflateWithHeader(data: ByteArray): ByteArray {
    ...
    // Zlib header (CMF + FLG)
    val cmf: Byte = 0x78 // CM=8 (deflate), CINFO=7 (32K window)
    val flg: Byte = 0x01 // FCHECK, no FDICT, FLEVEL=0
    ...
    while (offset < data.size) {
        ...
        if (btype != 0) {
            throw CompressionException(
                "Only stored blocks are supported. For full deflate, link to zlib.",
            )
        }
...
private fun parseGzipWrapper(data: ByteArray): ByteArray {
    ...
    if ((flags and 0x08) != 0) { // FNAME
        while (offset < data.size && data[offset] != 0.toByte()) offset++
        offset++ // Skip null terminator
    }

    if ((flags and 0x10) != 0) { // FCOMMENT
        while (offset < data.size && data[offset] != 0.toByte()) offset++
        offset++ // Skip null terminator
    }

    if ((flags and 0x02) != 0) { // FHCRC
        offset += 2
    }

    // Extract compressed data (exclude 8-byte trailer)
