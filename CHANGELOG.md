...
- **`@DispatchOn` and `@FramedBy` are now `@Retention(SOURCE)` (were `BINARY`),**
  for the same reason as `@ForwardCompatible`. Both are pure compile-time codegen
  directives read by KSP only off the same-round *source* symbol they annotate (or
  its same-module sealed parent); a field whose type is a `@DispatchOn`/`@FramedBy`
  message in another module delegates to that type's generated by-name codec rather
  than re-reading the annotation. SOURCE retention leaves codegen and every
  generated `Codec` byte-identical (verified by the snapshot suite) while keeping
  their `KClass` arguments — `type = ...::class` / `codec = ...::class` — out of
  bytecode and `@Metadata`. Both carry a `KClass` arg, so this closes the same
  proguard-core 9.3.2 abort for the (uncommon) case of a **nested** discriminator
  or framing-codec class, and trims the annotation from every message that uses
  them. `@DispatchValue` and `@UseCodec` **stay `BINARY`** — they are read off the
  *resolved* discriminator / value-class type, which can live in a dependency
  module, so they must survive to bytecode for a downstream module's KSP round.
  A new `SourceRetentionRegressionTest` asserts the SOURCE annotations never leak
  into a compiled fixture's bytecode or `@Metadata`.
