 ...
 */
@Target(AnnotationTarget.CLASS)
// SOURCE, not BINARY: `@FramedBy` is a pure compile-time codegen directive, read
// by KSP only off the same-round *source* symbol it annotates (or its same-module
// sealed parent — Kotlin pins sealed subclasses to the parent's module). A field
// whose type is a `@FramedBy` message in another module delegates to that type's
// generated by-name codec instead of re-reading the annotation, so SOURCE leaves
// codegen unchanged while dropping the annotation — and its `codec = ...::class`
// arg — from the `.class` file and Kotlin's `@Metadata`. This keeps a *nested*
// codec KClass (`codec = Outer.Inner::class`) out of the metadata that
// proguard-core 9.3.2 cannot resolve. Contrast with `@UseCodec` / `@ProtocolMessage`
// / `@EnumDefault`, which are read off *dependency-module* types and must stay BINARY.
 ...
 */
@Target(AnnotationTarget.CLASS)
// SOURCE, not BINARY: `@DispatchOn` is a pure compile-time codegen directive, read
// by KSP only off the same-round *source* sealed parent it annotates (or that
// parent reached through a same-module sealed supertype). A field whose type is a
// `@DispatchOn` message in another module delegates to that type's generated
// by-name codec instead of re-reading the annotation, so SOURCE leaves codegen
// unchanged while dropping the annotation — and its `type = ...::class` arg —
// from the `.class` file and Kotlin's `@Metadata`. This keeps a *nested*
// discriminator KClass (`type = Outer.Inner::class`) out of the metadata that
// proguard-core 9.3.2 cannot resolve. NB: `@DispatchValue` (read off the resolved,
// possibly cross-module discriminator type) must stay BINARY — see below.
@Retention(AnnotationRetention.SOURCE)
...
// BINARY (not SOURCE, unlike `@DispatchOn`): this is read off the *resolved*
// `@DispatchOn(type = ...)` discriminator type's property, which can live in a
// dependency module, so it must survive to that type's bytecode for a downstream
// module's KSP round to see it. It carries no `KClass` arg, so BINARY costs no
// proguard-core exposure.
@Retention(AnnotationRetention.BINARY)
