...
| 21 | `@UseCodec(Foo::class)` cross-platform resolution: **direct call, Kotlin linker resolves; KSP doesn't inspect platform actuals.** Generated code emits `Foo.decode(buffer, context)` by simple-name reference in commonMain; each platform's compilation links the call against that platform's `actual object Foo` declaration. The processor processes commonMain metadata only — forcing per-platform KSP runs to verify every actual exists would shatter the slice 0 doctrine ("single KSP run on commonMain metadata, generated codecs in commonMain") and add no safety the Kotlin Multiplatform compiler doesn't already enforce (the compiler guarantees an actual exists at link time and the symbol's signature is fixed by the expect declaration). Symmetric with hand-written multiplatform code: Kotlin consumers writing `Foo.decode(...)` in commonMain rely on the same linker-resolution semantics; generated code matches. Zero runtime cost (direct static dispatch on each platform — no reflection, no SPI lookup, no `ServiceLoader`). Slice 10e proves this end-to-end via the `RemoteCommandPayloadCodec` `expect`/`actual` fixture: identical wire bytes emitted from each platform's actual; KSP-emitted code references the `expect` symbol unaware of platform actuals. Driven by Stage H slice 10e. |
