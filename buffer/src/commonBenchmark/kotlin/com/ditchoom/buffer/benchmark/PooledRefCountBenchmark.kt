 ...
 *
 * Exists to settle one question with a number instead of an argument: `PooledBuffer.refCount` is a
 * plain non-atomic `Int`, which is why it cannot be shared across threads, which is why the
 * encode-once fan-out work carries a parallel atomic lifetime system beside it. Making the count
 * atomic removes that whole second system — but only if the atomic is affordable.
 *
